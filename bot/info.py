...
class Info(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        **Photos**
        ...
        """)
        self.embed = discord.Embed(title="**Help**", description=description, colour=discord.Colour.random())
        self.b = 1
        self.message = None
    ...
    async def helpp(self, ctx):
        embed = self.embed
        embed.set_footer(text=f"fun bot help § requested by {ctx.author}", icon_url=self.bot.user.avatar_url)
    ...
    async def pinggg(self, ctx, use: discord.User, times: int):
    ...
    @commands.command()
    async def u(self, ctx, *, user: discord.Member = None):
        if user is None:
            user = ctx.author
